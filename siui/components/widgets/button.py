...
class SiCheckBox(SiLabel):
    ...
    def __init__(self, parent, indicator_size: int = 20, text_gap: int = 8):
        super().__init__(parent)
        self.parent = parent
        s = max(8, min(40, int(indicator_size)))
        self._indicator_size = s
        self._text_gap = max(0, min(24, int(text_gap)))
        svg_inner = max(6, int(round(12 * s / 20)))
        r = max(2, min(6, s // 4))

        # 一个标签用于表现选中状态
        self.indicator_label = SiLabel(self)
        self.indicator_label.resize(s, s)
        self.indicator_label.setFixedStyleSheet(f"border-radius: {r}px")  # 注意：这里是固定样式表
        ...
        self.indicator_icon.resize(s, s)
        self.indicator_icon.setSvgSize(svg_inner, svg_inner)
        self.indicator_icon.load(svg_data.encode())

        # 创建选项按钮
        self.indicator = ABCButton(self)
        self.indicator.setCheckable(True)
        self.indicator.resize(s, s)
        self.indicator.setFixedStyleSheet(f"border-radius: {r}px")  # 注意：这里是固定样式表
        self.indicator.toggled.connect(self._toggled_handler)
        self.indicator.toggled.connect(self.toggled.emit)
    ...
    def adjustSize(self):
        ...
        g = self._text_gap
        self.resize(s + g + self.text_label.width(), max(32, s + 8))
    ...
    def setIndicatorSize(self, indicator_size: int):
        s = max(8, min(40, int(indicator_size)))
        if s == self._indicator_size:
            return
        self._indicator_size = s
        svg_inner = max(6, int(round(12 * s / 20)))
        r = max(2, min(6, s // 4))
        self.indicator_label.resize(s, s)
        self.indicator_label.setFixedStyleSheet(f"border-radius: {r}px")
        self.indicator_icon.resize(s, s)
        self.indicator_icon.setSvgSize(svg_inner, svg_inner)
        self.indicator.resize(s, s)
        self.indicator.setFixedStyleSheet(f"border-radius: {r}px")
        self._toggled_handler(self.isChecked())
        self.adjustSize()
