...
- **未完成 / 已完成分区**：未完成项始终在上方，已完成项在下方；**新添加的待办默认出现在未完成区最上方**。
- **勾选与取消勾选**：勾选后自动下移到已完成区；取消勾选时，按保存的**手动顺序**回到未完成区中的对应位置（不再仅用「创建先后」一种语义）。
- **顺序字段**：使用 `order_key` 记录清单内的手动顺序（含右键菜单的上移/下移/置顶，以及列表内**鼠标拖拽排序**）；使用 `completed_rank` 记录已完成区内部的显示顺序。旧数据中的 `created_order` 会在读取时兼容并视为 `order_key`。
- **拖拽与安全范围**：待办支持按住拖动调整顺序；目标位置会**限制在「未完成」或「已完成」各自分区内**，避免与分区规则冲突。

### 外观与深色模式

- **深色模式待办区**：修复深色模式下待办列表滚动区域、行背景仍透出浅色底、与主题卡片不一致的问题。
- **侧栏清单（深色模式）**：修复侧栏条目背景与文字对比度问题；当前选中清单的高亮改为**紫色系**（与全局偏蓝的主题强调色区分，避免视觉上偏绿/青）。
